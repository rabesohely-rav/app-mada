...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function exportData() {
    try {
      ...
      a.href = url;
      a.download = `depenses-${new Date().toISOString().slice(0, 10)}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      saveStatus.textContent = "Export téléchargé.";
      saveStatus.className = 'status-ok';
    } catch (error) {
      saveStatus.textContent = "Export impossible.";
      saveStatus.className = 'status-warn';
    }
  }
  function importData(event) {
    ...
    reader.onload = () => {
      try {
        ...
        if (!imported || !Array.isArray(imported.data)) throw new Error("fichier invalide");
        state = {
          displayCurrency: imported.displayCurrency === TARGET_CURRENCY ? TARGET_CURRENCY : DEFAULT_CURRENCY,
          eurToMgaRate: typeof imported.eurToMgaRate === "number" ? imported.eurToMgaRate : null,
          lastRateUpdate: typeof imported.lastRateUpdate === "string" ? imported.lastRateUpdate : null,
          lastRateSource: typeof imported.lastRateSource === "string" ? imported.lastRateSource : null,
          data: imported.data
        };
        saveState("Données importées.");
        renderTable();
      } catch (error) {
        saveStatus.textContent = "Import impossible : fichier invalide.";
        saveStatus.className = 'status-warn';
      } finally {
        importInput.value = "";
      }
